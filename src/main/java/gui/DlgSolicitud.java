...
public class DlgSolicitud extends JDialog implements ActionListener {
	...
	void listar() {
	    ...
	    try {
	        ...
	        for (Solicitud solicitud : lstSolicitudes) {
	            ...
	            imprimir("Fecha de registro.: " + solicitud.getFechaReg().toLocalDate());
	            ...
	            imprimir("Categoría.........: " + categoria.getDescripcion());
	...
	void adicionar() {
		String detalleSolicitud = txtIdSolicitud.getText(); 
		Actividad actividad = (Actividad) cboActividad.getSelectedItem();
		String archivoAdjunto = txtArchivoAdjunto.getText();
		
	    
	    String estado = "PE";
	    java.time.LocalDateTime fechaReg = java.time.LocalDateTime.now();
		
		EntityManager manager = JPAUtil.getEntityManager();
		
		try {
			Solicitud Solicitud = new Solicitud(null, actividad, estado, archivoAdjunto, fechaReg, detalleSolicitud);
			...
			mensajeError("Hubo un error en la Solicitud");
			e.printStackTrace();
		} finally {
			manager.close();
		}
